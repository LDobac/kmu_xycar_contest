...
def start():
    ...
    model.load_state_dict(torch.load('/home/nvidia/legacy.ckpt')['model_state_dicts']['model'])
    model.eval()
    ...
    while not rospy.is_shutdown():
        ...
        try:
            with open('/home/nvidia/fine_tuning.txt') as txt_file:
                y_range_start, y_range_end, lr_offset, left_coeff, right_coeff, P_coeff, D_coeff, I_coeff, P_seed, C_speed = np.array(txt_file.read().split()).astype(float)
        except:
            y_range_start = 15
            y_range_end = 0
        ...
        # less_than_zero_pts = (pts[:, 0] < 0.0).sum()
        # greater_than_zero_pts = (pts[:, 0] > 0.0).sum()
        # left_pts = less_than_zero_pts - (pts[:, 0] < tunnel_wall_x_range[0]).sum()
        # right_pts = greater_than_zero_pts - (pts[:, 0] > tunnel_wall_x_range[1]).sum()
        
        # if left_pts > tunnel_wall_threshold and right_pts > tunnel_wall_threshold:
        #     front_pts = (pts[:, 1] < tunnel_front_threshold).sum()
        #     front_pts += less_than_zero_pts - (pts[:, 0] < tunnel_front_x_range[0]).sum()
        #     front_pts += greater_than_zero_pts - (pts[:, 0] > tunnel_front_x_range[1]).sum()
